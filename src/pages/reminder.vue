...
export default {
  ...
  methods: {
    async reminder() {
      let self = this;
      this.$store.$auth.ctx.$axios.post('/rcms-api/1/me/pw/reminder',
      {
        email:this.email,
      },{
        ...
        if (response.data.errors.length == 0) {
          self.$store.dispatch('snackbar/setMessage', 'パスワード再設定メールを送信しました。');
          self.$store.dispatch('snackbar/snackOn');
          self.$router.push('/login');
        }
      }).catch(error => {
          this.$store.dispatch('snackbar/setError', 'メールアドレスが見つからないか、入力が間違っています。再度、ご確認ください。');
